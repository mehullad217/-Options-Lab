...
if live_mode:
    ...
    def compute_ttm_years(expiry_str: str) -> float:
        ...
        assuming expiry at 16:00 America/New_York.
        """
        now_ny = datetime.now(NY_TZ)
        # Parse expiry and set 4pm NY as expiry time
    ...
    def get_risk_free_rate(ttm_years: float) -> float:
        """Fetch a live U.S. Treasury yield (%) matching the option's maturity. Returns decimal form (e.g., 0.045 for 4.5%)."""
        # Map TTM to nearest treasury symbol
        if ttm_years <= 0.25:   # <= 3 months
            symbol = "^IRX"  # 13-week Treasury bill yield
        elif ttm_years <= 1:
            symbol = "^FVX"  # 5-year Treasury yield (slightly longer than 1y)
        elif ttm_years <= 5:
            symbol = "^TNX"  # 10-year Treasury yield
        else:
            symbol = "^TYX"  # 30-year Treasury yield

        try:
            data = yf.Ticker(symbol).history(period="1d")
            last_yield_percent = data.iloc[-1]['Close']
    ...
    try:
        ...
        if live_mode:
            ...
            if not expiries:
                ...
                interest_rate = round(get_risk_free_rate(TTM), 4)  # for calculations
                st.sidebar.success(f"Risk-Free Rate: {interest_rate:.3%}")  # for display in %
        if live_mode:
            chain = ticker.option_chain(expiry)
            # after you load chain = ticker.option_chain(expiry)
            ...
            # --- session state to persist user intent ---
            ...
            # choose the target we want to keep when expiry changes
            if st.session_state.prev_expiry != expiry:
                # first time on this expiry: try to keep prior strike; else default to ATM
            ...
            # (optional) tell user if we had to snap
            if abs(target_strike - strike_price) > 0.0001:
                st.sidebar.caption(f"Note: {target_strike:.2f} not listed for {expiry}; snapped to nearest {strike_price:.2f}.")

            #available_strikes = chain.calls["strike"].astype(float).tolist()
            #strike_price = st.sidebar.selectbox("Select Strike Price", sorted(available_strikes))

            row = chain.calls.loc[chain.calls["strike"] == strike_price]
            iv = float(row["impliedVolatility"].iloc[0]) if not row.empty else float("nan")
            MIN_VOL = 1e-4

            if not np.isfinite(iv) or iv <= MIN_VOL:
                volatility = st.sidebar.number_input("Volatility (Manual Fallback)", value=0.20, min_value=0.01, step=0.01)
            else:
                volatility = round(iv, 4)
            # Show the IV we pulled (per selected call strike) and the exact vol used
        ...
        #spot_price = round(ticker.history(period="1d")['Close'].iloc[-1], 2)

        #st.sidebar.success(f"📈 Spot Price: ${spot_price}")

        # try:
        #     implied_vol = opt_chain.calls.loc[opt_chain.calls['strike'] == strike_price, 'impliedVolatility'].values[0]
        #     volatility = round(implied_vol, 4)
        # except:
        #     volatility = st.sidebar.number_input("Volatility (Manual Fallback)", value=0.2)
...
with tab5:
    ...
    if strategy == "Iron Condor" or strategy ==  'Reverse Iron Condor':
        ...
        if view_mode == "📈 Line Payoff Chart":
            ...
            # expected_move = volatility * np.sqrt(TTM)
            # S_vals = np.linspace(
            #     spot_price * (1 - 2 * expected_move),
            #     spot_price * (1 + 2 * expected_move),
            #     50
            # )
            ...
            # ax.set_title("Iron Condor P&L Heatmap (Volatility-Adjusted)")
            # st.pyplot(fig)
            # # === Summary Table for Iron Condor ===
            # st.subheader("📋 Iron Condor Summary Table (Strike Spread vs. Profit/Loss)")
